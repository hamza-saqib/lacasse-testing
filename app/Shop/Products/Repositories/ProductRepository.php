...

{
...

    {
        ...
        try {
            if(isset($data['slug'])){
                if (Product::whereSlug($slug = str_slug($data['slug']))->exists()) {
                    $data['slug'] = $this->model->incrementSlug($slug);
                }
            }
...

    {
        ...
        if(isset($filtered['is_part'])){
            $filtered['is_part'] = implode(',', $filtered['is_part']);
        }
    ...
    public function findProductByIsPart(string $ispart) : Collection

    {

        return $this->model->whereRaw('find_in_set('.$ispart.','.'is_part)')->get();
