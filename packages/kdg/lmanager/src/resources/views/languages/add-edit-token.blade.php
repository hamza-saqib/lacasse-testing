				...
				<div class="row">
					<div class="col-md-6 offset-md-3">
						<ol class="breadcrumb">
							...
							<li><a href="{{URL_LANGUAGE_TOKEN_LIST}}">{{LanguageHelper::getPhrase('language_tokens')}}</a> </li>
				...
				<div class="panel panel-custom col-md-6 offset-md-3" >
					<div class="panel-heading">
						<div class="pull-right messages-buttons">
							<a href="{{URL_LANGUAGES_LIST}}" class="btn  btn-primary button" >{{ LanguageHelper::getPhrase('languages')}}</a>
						</div>
					<h1>{{ $title }}  </h1>
					</div>
					...
					@if ($record)
					 ...
					 <?php $button_name = LanguageHelper::getPhrase('update'); ?>
						{{ Form::model($record, 
						array('url' => URL_LANGUAGES_EDIT_TOKEN.'/'. $record->id, 
						'method'=>'patch','novalidate'=>'','name'=>'formLanguage')) }}
					@else
						{!! Form::open(array('url' => URL_LANGUAGE_TOKEN_ADD, 'method' => 'POST', 'name'=>'formLanguage ', 'novalidate'=>'')) !!}
					@endif

					 @include('lmanager::languages.token-form_elements', 
