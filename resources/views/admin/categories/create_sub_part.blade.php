            ...
            <form action="{{ route('admin.categories.store') }}" method="post" class="form" enctype="multipart/form-data" onsubmit="$('select').attr('disabled', false);">
                <div class="box-body">
                    ...
                    <div class="form-group">
                        ...
                        <select name="parent" id="parent" class="form-control select2" required="" disabled="disabled">
                            <option value="">-- Select --</option>
                            @foreach($carParts as $category)
                                <option value="{{ $category->id }}" @if($pid == $category->id) selected="selected" @endif>{{ $category->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
