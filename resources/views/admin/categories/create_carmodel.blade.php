            ...
            <form action="{{ route('admin.categories.store') }}" method="post" class="form" enctype="multipart/form-data" onsubmit="$('select').attr('disabled', false);">
                <div class="box-body">
                    ...
                    <div class="form-group">
                        ...
                        <select name="parent" id="parent" class="form-control select2" disabled="disabled">
                            <option value="">-- Select --</option>
                            @foreach($brands as $category)
                                <option value="{{ $category->id }}" @if($bid == $category->id) selected="selected" @endif >{{ $category->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                ...
                <div class="box-footer">
                    <div class="btn-group">
                        <a href="{{ url('admin/categories/'.$bid.'?type=carmodel') }}" class="btn btn-default">Back</a>
